...
1. Add a new enum variant to `DayOpt` in `args.rs` using the naming convention `DN` (no leading 0s!). Make sure that each day has its respective day number as value (Day7 = 7)!
2. Copy the `day0.rs` template (rename the file!), implement the solution and put your input in `inputs/dayN.txt` (no leading 0s!)
3. Add `pub mod dayN;` to `days.rs`
4. Add the new `DN` and `dayN` to the `exec_days!` macro in `main.rs`
